...
Arguments MonadMorphism {_ _ _ _ _ _}.

(* An higher-order functor that is also a monad transformer *)
  ...
  { hfmap_id :
    ...
    hfmap_nat :>
      forall (g h : Type -> Type)
        ...
        (F : forall T, g T -> h T) {F_Nat : MonadMorphism F},
        MonadMorphism (hfmap F);
    hfmap_proper :> HFmapProper f f_Hfunctor
  }.

(* Extra laws w.r.t. iter and lift *)
...
Class HFunctorLawsExtra (f : (Type -> Type) -> Type -> Type)
      ...
      `{f_WF_MonadTLaws : WF_IterativeMonadT f} :=
  { hfmap_lift :
      forall (T : Type) (g h : Type -> Type) `{g_IM : WF_IterativeMonad g} `{h_IM : WF_IterativeMonad h}
             (F : g ~> h) {F_MNT : MonadMorphism F}
             (t : g T),
        hfmap (f := f) F (EqmRMonadT.lift t) ≈{eq} EqmRMonadT.lift (F _ t);

    hfmap_iter : forall (g h : Type -> Type) `{g_IM : WF_IterativeMonad g} `{h_IM : WF_IterativeMonad h}
                   (F : forall T : Type, g T -> h T),
                 @MonadMorphism g h _ _ _ _ F ->
                 @IterMorphism g h _ F _ _ ->
                 @IterMorphism (f g) (f h) _ (@hfmap _ f_Hfunctor g h F)
                               _ _;
  }.
...
Require Import ITree.Basics.MonadState.
