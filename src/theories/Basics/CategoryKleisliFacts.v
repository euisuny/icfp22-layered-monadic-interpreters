...
Section BasicFacts.
  ...
  Context {m_EQM : EQM m m_EqmR}.

  Instance Proper_Kleisli_apply {a b} :
    Proper (eq2 ==> eq ==> eqmR eq) (@Kleisli_apply m a b).
  Proof.
...
Proof.
  ...
  eapply Proper_bind; auto; intros; subst; auto.
Qed.

Local Opaque bind ret eqmR.
...
Notation Proper_iter m a b :=
  (@Proper (Kleisli m a (sum a b)%type -> (Kleisli m a b))
           (pointwise_relation _ (eqmR eq) ==> pointwise_relation _ (eqmR eq))
           iter).
