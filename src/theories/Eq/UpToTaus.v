...
Lemma eq_itree_conj {E} {R S} {RS RS'} :
  forall (t : itree E R) (s : itree E S),
    eq_itree RS t s ->
    eq_itree RS' t s ->
    eq_itree (RS /2\ RS') t s. 
Proof.
  repeat red.
  ginit. gcofix CIH. intros * EQ EQ'.
  ...
  hinduction EQ before CIH; subst; intros; pclearbot; simpl.

  - gstep.
    inv EQ'; auto. constructor. auto.
  - gstep. constructor. gbase.
    eapply CIH; inv EQ'; try inv CHECK; auto. pclearbot. auto.
  - assert (EE := eqitF_inv_VisF _ _ _ _ _ EQ'); pclearbot.
    gstep. constructor. intros. gbase. apply CIH; eauto.
  - inv CHECK.
  - inv CHECK.
Qed.
