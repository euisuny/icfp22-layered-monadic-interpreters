...
Require Import ExtLib.Structures.Monad Proofmode.
...
From ITree Require Import
     ...
     Events.State.
From ITree Require Import EqmR.Monads.ErrorT.
...
(* TODO: No-Failure proof
    Remove abort, compiler should compile aborting imp to skip  *)
...
Definition map_reg : Reg ~> mapE reg 0 :=
  fun _ e =>
    match e with
    | GetReg x => LookupDef x
    | SetReg x v => Insert x v
    end.

Definition map_memory : Memory ~> mapE addr 0 :=
  fun _ e =>
    match e with
    | Load x => LookupDef x
...
Definition handle_reg {E} : Reg ~> stateT registers (itree E) :=
  fun (T : Type) (X : Reg T) => handle_map (map_reg T X).

Definition handle_mem {E} : Memory ~> stateT memory (itree E) :=
  fun (T : Type) (X : Memory T) => handle_map (map_memory T X).
...
Instance registers_inhabited : inhabited registers.
  constructor; exact nil.
Qed.

Instance memory_inhabited : inhabited memory.
  constructor; exact nil.
Qed.

Existing Instance compose_HFunctor.
Existing Instance compose_WF_HFunctor.

(* If we want to allow for a general context for events, we need to enforce a
  certain order on the subevent constraints. *)
Definition interp_asm'
           {C D E F A} `{Abort +? C -< D} `{Reg +? D -< E} `{Memory +? E -< F}
           (t : itree F A) : stateT memory (stateT registers (failT (itree C))) A :=
  interp (T := fun x => stateT memory (stateT registers x)) (over handle_abort)
         (interp (T := stateT memory) (over handle_reg)
                 (interp (T := fun x => x) (over handle_mem) t)).

Definition interp_asm
           {C D E F A} `{Abort +? C -< D} `{Memory +? D -< E} `{Reg +? E -< F}
           (t : itree F A) : stateT registers (stateT memory (failT (itree C))) A :=
  interp (T := fun x => stateT registers (stateT memory x)) (over handle_abort)
         (interp (T := stateT registers) (over handle_mem)
                 (interp (T := fun x => x) (over handle_reg) t)).

(* Given a concrete set of events in the program, the order of the sum of events
 does not matter. *)
Definition interp_asm_concrete {A} (t : itree (Abort +' Reg +' Memory) A)
  : stateT memory (stateT registers (failT (itree void1))) A :=
  interp (T := fun x => stateT memory (stateT registers x)) (over handle_abort)
         (interp (T := stateT memory) (over handle_reg)
                 (interp (T := fun x => x) (over handle_mem) t)).
...
Section InterpAsmProperties.
  ...
  Context {HasAbort : Abort +? C1 -< C2}.
  Context {HasMemory : Memory +? C2 -< C3}.
  Context {HasReg : Reg +? C3 -< E}.

  Opaque interp. Opaque eqmR.

  (** This interpreter is compatible with the equivalence-up-to-tau. *)
  #[global] Instance eutt_interp_asm {R}:
    Proper (eqmR eq ==> eqmR eq) (interp_asm (A := R)).
  Proof.
    repeat intro.
    unfold interp_asm.
    ...
    (* FIXME : Can we get around this explicit chain of applications? *)
    eapply HProper1, HProper0, HProper; eauto.
  Qed.
  ...
  Lemma interp_asm_bind: forall {R S} (t: itree E R) (k: R -> itree E S),
