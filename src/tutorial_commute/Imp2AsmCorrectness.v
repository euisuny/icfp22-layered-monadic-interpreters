...
Section Bisimulation.
  ...
  Section RAB.
    ...
    Proof.
      ...
      { specialize (H env _ eq_refl);
          specialize (H0 reg _ eq_refl mem _ eq_refl);
  ...
  Lemma bisimilar_bind' {EE A A' B C} (RAA' : A -> A' -> Prop) (RBC: B -> C -> Prop):
    forall (t1 : itree (ImpFail +' ImpState +' EE) A) (t2 : itree (Reg +' Memory +' Abort +' EE) A') ,
      bisimilar (E := EE) RAA' t1 t2 ->
      forall (k1 : A -> itree (ImpFail +' ImpState +' EE) B) (k2 : A' -> itree (Reg +' Memory +' Abort +' EE) C)
  ...
  Lemma bisimilar_iter {EE A A' B B'}
        (R : A -> A' -> Prop)
        (S : B -> B' -> Prop)
      (t1 : A-> itree (ImpFail +' ImpState +' EE) (A + B)) (t2 : A' -> itree (Reg +' Memory +' Abort +' EE) (A' + B')):
  ...
  Lemma sim_rel_get_tmp0:
    forall EE n l l' g_asm g_imp v,
      sim_rel l' n (g_imp,v) (g_asm, (l, tt)) ->
            (interp_asm (C := EE) ((trigger (GetReg n))) l g_asm)
            ≈     (Ret (inr (g_asm, (l, v)))).
  Proof.
    intros.
    unfold interp_asm.
    ...
    - Proofmode.f_equiv.
      ...
      tau_steps. rewrite <- itree_eta. Unshelve. 6 : intros; eapply ret. cbn.
      rewrite Eq.bind_ret_l. 6 : eapply inr. cbn. reflexivity. reflexivity.
      cbn.
      unfold MonadFail.Kleisli_MonadIter. cbn. setoid_rewrite unfold_iter.
      tau_steps.
      apply eqit_Ret.
...
Section Correctness.
  ...
  Definition expr_bisim {EE} l n (ma:stateT env (Imp.failT (itree EE)) value)
             (mb:stateT registers (stateT memory (Imp.failT (itree EE))) unit) :=
    forall env mem, Renv env mem ->
                   eqmR (m := Imp.failT (itree EE)) (sim_rel l n) (ma env) (mb l mem).
  ...
  Proof with (cbn; autorewrite with itree).
    ...
    { bind_cong; [ do 3 ibind | ..];
      ...
      Proofmode.f_equiv; [ done | ..].
    { bind_cong. iignoretrigger.
            iproper; eapply HProper1, HProper0;
                iignoretrigger; eapply HTrigger0; try typeclasses eauto.
        all :
          intros; subst; bind_cong.
          itrigger. iproper. eapply HProper1, HProper0.
                  eapply HTrigger.
    ...
    - (* Plus case *)
      ...
      { intros.
        ...
        eapply eutt_clo_bind.
        { apply IHe2; eauto.
          eapply (@sim_rel_Renv l n _ (fst (snd a2)) (snd (snd a2)) (fst a1) (snd a1)).
